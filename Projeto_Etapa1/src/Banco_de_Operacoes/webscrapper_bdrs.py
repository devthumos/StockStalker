...
def click_datapage(driver, i):
    try:
        driver.find_element_by_xpath(f"//main[@id='main-2']//section[@class='container']//div[@class='w-100 justify-center']//ul[@class='pagination mb-0']//li[@data-page='{i}']//a[@role='button']").click()
        print(f"\tClickando {i}")
        time.sleep(1)
    except:
        try:
...
def get_code(driver, codes):
    try:
        ...
        print(hrfs_finded)
        print(len(hrfs_finded))
        for i in hrfs_finded:  # para testes
            codes.append(i["href"][6:])
    except:
        return False
