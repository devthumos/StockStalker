...
def click_datapage(driver, i):
    try:
        driver.find_element_by_xpath(f"//main[@id='main-2']//div[@class='white']//section[@class='container']//div[@class='w-100 justify-center']//ul[@class='pagination mb-0']//li[@data-page='{i}']//a").click()
        print(f"\tClickando {i}")
        time.sleep(1)
    except:
        try:
...
def get_code(driver, codes):
    try:
        html_content = driver.find_element_by_xpath(f"/html/body/main/div[3]/section/div[2]")
        html_content = html_content.get_attribute("outerHTML")

        doc = BeautifulSoup(html_content, "html.parser")
        hrfs_finded = doc.find_all("a")
        print(len(hrfs_finded))  # teste

        for i in hrfs_finded:  # para testes
            codes.append(i["href"][21:])
    except:
        return False
...
for i in range(1, 24):
    ...
    #  TESTE
    # while valido_click_datapage == False:
    #     valido_click_datapage = False if click_datapage(driver, i) == False else True
    #     time.sleep(1)
